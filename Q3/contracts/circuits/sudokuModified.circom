...
template sudoku() {
    ...
    component rp[9][9], rP[9][9];
    
    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            rp[i][j] = RangeProof(32); //making an instance of template RangeProof
            rp[i][j].in <== puzzle[i][j]; //assigning the input signal in
            rp[i][j].range[0] <== 0; //lower bound
            rp[i][j].range[1] <== 9; //upper bound
            assert(rp[i][j].out == 1); //checking if the instance gives value of out as 1

            rP[i][j] = RangeProof(32);
            rP[i][j].in <== solution[i][j];
            rP[i][j].range[0] <== 0;
            rP[i][j].range[1] <== 9;
            assert(rP[i][j].out == 1);
